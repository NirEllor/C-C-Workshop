...
Node* get_node_from_database(MarkovChain *markov_chain, void *data_ptr)
{
    ...
    {
        if (markov_chain->comp_func(first_node->data->data,
                                    data_ptr) == 0) // Found
        {
            return first_node;
        }
                              ...
                              MarkovChain *markov_chain)
{
    ...
    for (int i = 0; i < first_node->cur_freq_size; ++i)
    {
        ...
        ->frequencies_list[i].markov_node->data,
                   (second_node->data)) == 0)
        {
...
void free_markov_chain(MarkovChain ** ptr_chain)
{
    ...
    for (int i = 0; i < db_size; ++i)
    {
        ...
        (*ptr_chain)->free_data(first_node->data->data); // Free string
...
MarkovNode* get_first_random_node(MarkovChain *markov_chain)
{
    ...
    } while (markov_chain->is_last(chosen_node->data->data) == true);
...
MarkovNode* get_next_random_node(MarkovNode *state_struct_ptr)
{
    ...
    int number_random = get_random_number(
            state_struct_ptr->freq_sum);
